...
export const GroupProvider = ({ children }: { children: React.ReactNode }) => {
  ...
  const scheduleSimUpdates = useCallback(() => {
    ...
    const tick = () => {
      try {
        ...
        const base = useAppStore.getState().userLocation || (currentMembers.find((m) => m.isSelf)?.position ?? { lat: 23.1765, lng: 75.7884 });
        currentMembers.forEach((m) => {
          ...
          const headingRad = Math.atan2(nextLng - m.position.lng, nextLat - m.position.lat);
          const headingDeg = (headingRad * 180) / Math.PI;
          const normalized = (headingDeg + 360) % 360;
          const prevPath = m.path || [{ lat: m.position.lat, lng: m.position.lng, ts: m.lastUpdated || Date.now() }];
          const nextPath = [...prevPath, { lat: nextLat, lng: nextLng, ts: Date.now() }];
  ...
  const ensureSelfMember = useCallback((pos: { lat: number; lng: number } | null) => {
    ...
    if (existingSelf) {
      const basePath = existingSelf.path || [{ lat: existingSelf.position?.lat || (pos?.lat || 23.1765), lng: existingSelf.position?.lng || (pos?.lng || 75.7884), ts: existingSelf.lastUpdated || Date.now() }];
      const nextPos = pos || existingSelf.position || { lat: 23.1765, lng: 75.7884 };
  ...
  const joinGroup = useCallback((code: string) => {
    ...
    try {
      ...
      if (!existingTs || prevCode !== code) {
        localStorage.setItem(STORAGE_KEYS.groupJoinedAt, Date.now().toString());
      }
    } catch {}
  ...
  }, [userId, setUserIdInStore, setGroupInStore, clearMembersInStore, setUserLocationInStore, ensureSelfMember, initializeMockMembers, scheduleSimUpdates]);
  ...
  }, [ensureSelfMember, initializeMockMembers, scheduleSimUpdates, setGroupInStore, clearMembersInStore]);
  ...
  useEffect(() => {
    ...
    locationService.updateLocation({ latitude: userLocation.lat, longitude: userLocation.lng }).catch(() => {});
  }, [userLocation]);
    ...
    const poll = async () => {
      try {
        ...
        res.locations.forEach((loc) => {
          ...
          const existing = currentState.members.find((m) => m.id === loc.userId);
          if (existing) {
            const basePath = existing.path || [];
            const nextPath = [...basePath, { lat: pos.lat, lng: pos.lng, ts: Date.now() }];
