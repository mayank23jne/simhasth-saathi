...
export function normalizeAuthData(res: any): NormalizedLoginData {
  ...
  const membersRaw: any[] = Array.isArray(data.groupMembers)
    ? data.groupMembers
    : Array.isArray(data.group_members)
    ? data.group_members
    : [];
...
export const authService = {
  async registerAdmin(payload: RegisterUserPayload): Promise<AuthResponse> {
    const res = await apiFetch<AuthResponse, RegisterUserPayload>(
    ...
    const user = (res as any)?.data?.user as
      | { id?: string | number; isAdmin?: boolean }
      | undefined;
    if (token && user?.id != null) {
      setAuth(token, user?.isAdmin ? "admin" : "member", {
  ...
  async registerMember(payload: RegisterMemberPayload): Promise<AuthResponse> {
    const res = await apiFetch<AuthResponse, RegisterMemberPayload>(
  ...
  async loginAdmin(payload: LoginPayload): Promise<AuthResponse> {
    const res = await apiFetch<AuthResponse, LoginPayload>(
      "/api/auth/login-user",
      {
        method: "POST",
        body: payload,
    ...
    if (norm.token && norm.user?.id) {
      setAuth(norm.token, norm.user.isAdmin ? "admin" : "member", {
        userId: norm.user.id,
      });
    }
    return res;
  ...
  async verifyOtp(
    ...
    const res = await apiFetch<AuthResponse, VerifyOtpPayload>(
      "/api/auth/verify-otp",
      {
        method: "POST",
        body: payload,
    ...
    const nestedUser = (res as any)?.data?.user as
      | { id?: string | number; isAdmin?: boolean }
      | undefined;
    ...
    const userId =
      nestedUser?.id != null
        ...
        ? String(flatUserId)
        : undefined;
    const userType: "admin" | "member" = nestedUser?.isAdmin
      ? "admin"
      : "member";
  ...
  async getQrScanDetails(id: string | number): Promise<AuthResponse> {
    const res = await apiFetch<AuthResponse>(`/api/qr/scan/${id}`, {
      method: "GET",
    });
