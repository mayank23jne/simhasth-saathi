...
export const QRScanner: React.FC<QRScannerProps> = ({
    ...
    if (parsed.kind === 'member_card') {
      return {
        ...
        details: {
          ...
          ...(typeof parsed.latitude === 'number' ? { latitude: parsed.latitude } : {}),
          ...(typeof parsed.longitude === 'number' ? { longitude: parsed.longitude } : {}),
          ...(typeof parsed.locationName === 'string' ? { locationName: parsed.locationName } : {}),
  ...
  const onScanUpdate = useCallback((err: any, result: any) => {
    ...
    if (mode === 'member' && mapped && mapped.type && mapped.type !== 'member') {
      mapped = null;
    }
    if (mode === 'lost_found' && mapped && mapped.type && mapped.type !== 'lost_person') {
      mapped = null;
    }
  ...
  }, [hasScanned, close, onScanResult, mode]);
  ...
  const handleFileChange = useCallback(async (e: React.ChangeEvent<HTMLInputElement>) => {
    try {
      ...
      try {
        ...
        const ReaderCtor: any = (ZX as any).BrowserQRCodeReader || (ZX as any).BrowserMultiFormatReader;
        ...
        if (text) {
          const parsed = tryParseQR(text);
          let mapped = parsed ? mapParsedToResult(parsed) : null;
          if (mode === 'member' && mapped && mapped.type && mapped.type !== 'member') mapped = null;
          if (mode === 'lost_found' && mapped && mapped.type && mapped.type !== 'lost_person') mapped = null;
          if (mapped) {
            setHasScanned(true);
            try { if (navigator.vibrate) navigator.vibrate(15); } catch {}
            toast.success('QR स्कैन हुआ');
          ...
          <div className="rounded-lg overflow-hidden border bg-black/5 relative">
            ...
            {cameraError ? (
              <div className="p-4 text-sm text-destructive">
                {t('cameraError') || 'Camera error. Please allow camera permission or enter code manually'}
              </div>
            ) : (
              <BarcodeScanner
                ...
                torch={torch}
                onError={(e: any) => setCameraError(String(e?.message || e) || 'Camera error')}
              />
            )}
          </div>
          <Button variant="outline" onClick={close} className="w-full">{t('back') || 'Back'}</Button>
        </div>
      </DialogContent>
    </Dialog>
  );
